...
fun saveBitmapAsPdf(context: Context, bitmap: Bitmap, fileName: String) {
    ...
    uri?.let {
        try {
            ...
            if (outputStream != null) {
                ...
                Toast.makeText(context, "Error accessing output stream", Toast.LENGTH_SHORT).show()
            }
        } catch (e: IOException) {
            Log.e("PDF", "Error: $e")
            e.printStackTrace()
        ...
        Toast.makeText(context, "Error creating URI", Toast.LENGTH_SHORT).show()
    }
...
@Composable
fun ExportExamDetailsScreen(    //IN navigation used
    navigateBack: () -> Unit,
    modifier: Modifier = Modifier,
    examViewModel: ExamDetailsViewModel = hiltViewModel()
) {
